...
the {\em ribbon move} denoted $\mu_{11}$ (which is in principle redundant). Using  $\mu_{11}$
makes all the moves coming from plane duality (the starred moves), except for $\mu_{20}^\star$,
redundant.
\end{abstract}
...
\label{theo:theorem}
 ...
 to the coin are shown as small black disks.
...
This paper is in confluency of two deep research passions of the author, apparently very far apart:
the topological study of closed orientable 3-manifolds and the combinatorial study of plane 
graphs.  The result proved here provides a hint, in the spirit of Conway's quotation, to effectively
enumerate once each closed orientable 3-manifolds. 
Blinks are easy to constuct, their isomorphism problem can be solved by a polynomial
algorithm (by finding by means of a few fixed conventions (lexicography), a numerical {\em code} for it). 
Our {\em canonical form of a closed oriented
3-manifold} is defined to be the blink inducing it which has
a minimum number of edges and minimum code.
...
\caption{\sf A proof that in the presence of $\pm r_2^{\pm 1}$, $a_3 \equiv A_3$ and $a_4 \equiv A_4$ }
\label{fig:proofequivalencea3A3a4A4}
\end{center}
\end{figure} 
\begin{lemma}
 In the presence of  $r_2^{\pm 1}$, moves $\pm a_3$ and $\pm A_3$ are equivalent and so are
 $\pm a_4$ and $\pm A_4$.
\end{lemma}
\begin{proof}
...
language and the respective blinks superimposed. The result follows by removing the superimposed moves leaving only the
blink moves which are redraw up to isotopy in the lower part of the figure.
\end{proof}
...
\caption{\sf Blackboard framed version of Martelli's revised calculus with $a_3$, $a_4$ replacing $A_3$, $A_4$. 
In the upper part of the figure, links \& blinks are superimposed implying 
moves for the blink-coin calculus in the lower part of the figure, 
...
Moves $\pm\mu_{11}^{\pm 1}$ are redundant because Martelli's calculus is in $\mathbb{R}^3$. We include them 
...
\end{theorem}
\begin{proof}
 We work not a the blink level, but at the level of its associated blackboard framed link.
 ...
 curls with distinct sign and the same rotation number. Use the the appropriate ribbon move to
 obtain two curls with the distinct signs and distinct rotation number. Now apply Whitney's trick
 (see page 30 of \cite{lins2007blink}) to cancel these curls. The net effect in the correponding blink
 is that it is obtained from the initial blink by dualizing and interchanging black and gray
...
\caption{\sf Reduced 36 move blink-coin calculus}
