...
\subsection{Statement of the Teorem}
...
definitions which are new and that will be 
used throughout the paper. Moreover, I present a short historic 
overview of the known results needed.

\subsubsection{Formal definitions for the various types of links}

We need to define various types of links:
link, 
decorated general position link projection,
framed link, blackboard framed link, bflink, blink, hflink, r-bflink, g-blink.
...
\raisebox{-3mm}{\includegraphics[width=7cm]{A.figs/reidemeistermoves.pdf}}.
...
with bflinks in Fig. \ref{fig:bflinkandlinktogether}
...
A {\em blackboard framed link} is a projection of a framed link prepared by isotopy so that 
...
\cite{kauffman1994tlr}. In this work, it is central: every framed link I use is 
blackboard framed. 

A {\em bflink} is the projection of the base link of a blackboard framed link. 
Thus a bflink is a 
decorated general position link projection, with no numbers attached. 
...
component of the link. A filling algorithm algorithm applies component by component, is named 
{\em $(\pm p,q)$-Dehn filling}, and is explained in Fig. \ref{fig:General_pq_DehnFilling}. 
...
The form I use for the solid tori is convenient because of its simple relation with bflinks.
It is inspired in the lucid account by J. Stillwell of the theorem 
given in \cite{stillwell1993classical}.

\begin{figure}[H]
\begin{center}
\includegraphics[width=13.0cm]{A.figs/General_pq_DehnFilling.pdf} 
\caption{\sf Dehn's $(\pm p,q)$-filling of a toroidal hole $T$ and its particular case
where $q=1$, yielding a well defined bflink (or link) as input for an algorithm to produce
a 3-manifold:
suppose that $\frac{\pm p}{q}$ is an irreducible fraction, where the case 
$q$=0, $p=1$ is permitted. 
Consider a solid torus in $\mathbb{S}^3$ as the homeomorphic image of 
$\mathbb{S}^ 1  \times[-\epsilon,+\epsilon]\times[-\delta,+\delta]$, denoted by $T$,
where $\epsilon$ and $\delta$ are arbitrarily small fixed positive constants. $T$ can be
changed by ambient isotopy so that for each $s \in \mathbb{S}^1$,
$T$ restricted to $\{s\} \times [-\epsilon,+\epsilon] \times [-\delta,+\delta]$,
is a rectangle which is orthogonal to the projection $xy$-plane. Therefore,
the projection of its restriction to 
$\mathbb{S}^ 1  \times[-\epsilon,+\epsilon]\times \{\delta\}$ is a blackboard framed
link whose base link $\lambda$ is defined by the restriction of $T$ to  
$\mathbb{S}^ 1  \times\{0\} \times \{\delta\}.$ 
The important fact is that $\lambda$ does not touch the boundary of its immersed band
forming the projection of the blackboard framed link. Let $\theta$ be
a curve that bounds a disk in $T$ but not in its boundary, $\partial(T)$.
The curve $\pm p.\theta + q.\lambda$ is isotopic to a simple curve in $\partial(T)$.
Map this curve homeomorphically to $m$, a curve in a canonical solid torus $T'$
in $\mathbb{R}^3$ so that it bounds a disk in $T'$ but not in $\partial(T')$.
This homeomorphism is univocally extensible to identify $\partial(T)$ and $\partial(T')$
thus closing the toroidal hole. Note that the whole situation is defined
by the knot $K \equiv \mathbb{S}^ 1  \times \{0\} \times \{0\}$ and the surgery coeficients
$(\pm p,q)$. In his original paper Lickorish only use Dehn fillings of type $(\pm p,1)$,
corresponding to what he calls {\em honesty surgeries}. For such surgeries each component $K$ 
may be curl-adjusted so as the whole link becomes a bflink where each $\pm p$ is given by the 
linking number of the boundary components of the corresponding band. 
This establishes an algorithm to obtain a 
well defined closed, oriented, connected 3-manifold $\mathbb{M}^3$ 
from a bflink, or from a blink, as input. By Lickorish's theorem, every such manifold arises
in this way. Even if the blink is not connected, $\mathbb{M}^3$  is connected: in general,
it is the connected sum of the manifolds induced by the connected components of the blink
which induces it.
}
...
In this work the handle slide move is applied only to bflinks, so there is no issue
about surgery coefficients.
Given $K$ and $K'$ two distinct components of a
bflink start by making a close parallel copy of $K'$ in such a way that 
...
this hieroglyphic move is non-local because the outside of the hieroglyph changes, and 
since there are infinite exteriors, there are infinite Kirby's band moves.
}
\label{fig:handleslide}
\end{center}
\end{figure} 

\subsubsection{Fenn-Rourk reformulation of Kirby's calculus}
...
However, Martelli's theorem and the coin calculus on blinks make it conceivable. 

\begin{figure}
\begin{center}
\includegraphics[width=16.5cm]{A.figs/Martelli.pdf} 
\caption{\sf Martelli's calculus on brflinks.
The links are in $\mathbb{R}^3$ and are endowed with an irreducible
fraction $\frac{\pm p_K}{q_K}$ assigned to component $K$.
...
in terms of the above 5 local moves. 
...
bflink moves leaving only the blink moves which are redrawn up to isotopy,
...
\end{corollary}
\begin{proof}
We work in the language of bflinks.
...
theory developed in \cite{kauffman1994tlr}. There are 3 independent implementations 
...
and is available upon request. The sequence of WRT-invariants of a closed oriented 
