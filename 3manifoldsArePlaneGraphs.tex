...
\label{theo:theorem}
 ...
 In Section \ref{sec:objective} the objective of this work from an epistemological
 point of  view is discussed. The proof of the Theorem \ref{theo:theorem} 
 ...
 In Section \ref{sec:conclusion} I display a census (no nisses, no duplicates) of
...
We need to define vaious tyoes of links:
...
component of the link. The same algorithm applies component by component, is named 
...
\caption{\sf Dehn's $(\pm p,q)$-filling of a toroidal hole $T$: 
...
changed by ambient isotopy so that the projection of its restriction to 
...
The important fact is that $\lambda$ does not touch the boundary of 
the blackboard framed link. Let $\theta$ be
...
As a consequence, each closed orientable 3-manifold $\mathbb{M}^3$ 
can be encoded by a link in $\mathbb{S}^ 3$ 
...
from the fractional framed link, act as follows: 
after removing from $\mathbb{M}^3$
a solid torus in the form of a homeomorphic image of
$(\mathbb{S}^ 1 \times[-\epsilon,+\epsilon]\times[-\delta,+\delta])_i$ 
of the $i$-th link we are left with 
...
link start by making a close parallel copy of $K'$ in such a way that 
it has linking number 0 with its originator $K'$.
Let $K''$ be the connected sum of $K$ and the copy of $K'$. 
The connected sum is defined by a {\em band} which is a
...
\subsection{The work from an epistemological point of view}
\label{sec:objective}
...
becomes a subtle class of plane graphs. The exposition is complete and elementary seeking
to reach both audiences: topologists and combinatorialists. We feel that this 
